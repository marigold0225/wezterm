...
M.set_process_name = function(s)
  ...
  local b = a:gsub("%.exe$", "")
  return custom_process_names[b] or b
  -- return a:gsub("%.exe$", "")
end
...
M.set_title = function(process_name, static_title, active_title, max_width, inset)
  ...
  local icon = " "
  if process_name == "PowerShell" then
    icon = " "
  elseif process_name == "Arch" then
    icon = " "
  end
  ...
  elseif static_title:len() > 0 then
    title = "󱁇 " .. static_title .. " ~ " .. " "
  else
    title = "󱁂 " .. active_title .. " ~ " .. " "
  end
